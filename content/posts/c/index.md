...
循环输入。每组数据输入一个 int 类型的正数 x，求 x 的相反数（注意：不能用负号）。当没有任何输入时，程序结束。
...
循环输入，每输入一个正整数 n (n ≤ 12)，输出 1 _ 2 _ 3 _ ... _ n 的值，当没有任何输入时，结束程序。

### 解题思路

- 阶乘：f(n) = 1 _ 2 _ 3 _ ... _ n。
- 递推式：f(n) = n \* f(n - 1) (n > 0)。
...
- a = a - b：执行完毕后，相当于 a 的值变成了 a + b - a，即原先 b 的值。
...
- 当 n 取最大值 65535 时，n _ (n + 1) = 65535 _ 65536 = (2 ** 16 - 1) \* 2 ** 16 = 2 ** 32 - 2 ** 16，而 int 能够表示的最大值为 2 \*\* 31 - 1，所以产生了溢出，变成了负数。
